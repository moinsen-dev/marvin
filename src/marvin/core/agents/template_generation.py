...
class TemplateGenerationAgent(Agent):
    ...
    def __init__(
        self, name: str = "template_generation", config: dict[str, Any] | None = None
    ):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        additional_context = await self._prepare_additional_context(
            task, feature, prd, codebase, **kwargs
        )
    ...
    async def _prepare_additional_context(
        ...
        **kwargs: Any,
        ...
        business_domain = (
            prd.title.split(":")[1].strip() if ":" in prd.title else prd.title
        )
        context["business_domain"] = business_domain

        # Add assumptions
        context["assumptions"] = (
            f"Feature '{feature.name}' can be implemented independently."
        )
